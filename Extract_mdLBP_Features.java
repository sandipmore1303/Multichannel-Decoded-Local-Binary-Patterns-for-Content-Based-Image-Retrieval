 ...
 * @author sam
 */
public class Extract_mdLBP_Features {
   
     public    Extract_mdLBP_Features() throws IOException, WriteException{
      ...
      File file1 = new File("mdLBP_Features.txt");
      ...
      if (file.isDirectory()) {
          ...
          for (int i = 0; i < list.length; i++) {
              ...
              mdM ma=new mdM(lbp);
              mdLBP malbp=new mdLBP(ma);
              malbp.compute_Hist_mdLBP_t1();
              for(int ws_c=0;ws_c<malbp.Histogram_mdLBP.length;ws_c++){
                //jxl.write.Number number = new jxl.write.Number(ws_c, i, malbp.Histogram_maLBP[ws_c]);
                //sheet.addCell(number); 
                  fileWriter.write(Double.toString(malbp.Histogram_mdLBP[ws_c])+" ");
              }
              fileWriter.write("\n");
     ...
     Extract_mdLBP_Features elpbf=new Extract_mdLBP_Features();
     }
}
