 ...
 * @author sam
 */
public class IR_mdLBP {
    
    double distances[];
    mdLBP ip_image;
    public IR_mdLBP(int noofimages) throws FileNotFoundException, IOException{
        //take ip image
        ...
        mdM ma = new mdM(lbp);
        ip_image = new mdLBP(ma);
        ip_image.compute_Hist_mdLBP_t1();
        
    //compare it with every malbp feature in database . find distance of it from every feature
        //1 read feature file
        //store the
         BufferedReader br = new BufferedReader(new FileReader("mdLBP_Features.txt"));
         int count=0;
         String line=null;
         String[] split =null;
         ...
         for(int i=0;i<1000;i++){
             ...
             for(int j=0;j<split.length;j++){
                  ...
                  distances[i] +=(ip_image.Histogram_mdLBP[j]-features[j])*(ip_image.Histogram_mdLBP[j]-features[j]);
             }
